...
namespace CustomAircraftTemplateJAS39
{
    ...
    public class FlagswitchScript : MonoBehaviour
    {
        public Texture swedenTexture;    // Texture for Swedish flag
        public Texture czechTexture;    // Texture for Czech flag

        public Material flagMaterial;

        public void SwitchFlagState(int st)
        {
            if (st == 0)
            {
            ...
            if (st == 1)
            {
                ...
                flagMaterial.mainTexture = swedenTexture;
            }
            if (st == 2)
            {
                gameObject.SetActive(true);
                flagMaterial.mainTexture = czechTexture;
            }
        }
